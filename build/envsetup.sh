...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/BiancaProject-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.lineage.device | grep -q "$BIANCA_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $BIANCA_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.lineage.device | grep -q "$BIANCA_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.lineage.device | grep -q "$BIANCA_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
